...
Install `types-aiobotocore` for `Route53GlobalResolver` service.

```bash
# install with aiobotocore type annotations
python -m pip install 'types-aiobotocore[route53globalresolver]'

# Lite version does not provide session.client/resource overloads
# it is more RAM-friendly, but requires explicit type annotations
python -m pip install 'types-aiobotocore-lite[route53globalresolver]'
...
```bash
python -m pip uninstall -y types-aiobotocore-route53globalresolver
```
...
Type annotations and code completion for  `#!python session.create_client("route53globalresolver")` as [Route53GlobalResolverClient](./client.md)
...
from `#!python session.create_client("route53globalresolver").get_paginator("...")`.
...
def get_list_access_sources_paginator() -> ListAccessSourcesPaginator:
    return client.get_paginator("list_access_sources"))
```
