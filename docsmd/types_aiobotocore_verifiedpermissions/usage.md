...
async with session.create_client("verifiedpermissions") as client:  # (1)
    result = await client.create_identity_source()  # (2)
```

1. client: [VerifiedPermissionsClient](./client.md)
2. result: [:material-code-braces: CreateIdentitySourceOutputTypeDef](./type_defs.md#createidentitysourceoutputtypedef) 
...
from types_aiobotocore_verifiedpermissions.type_defs import CreateIdentitySourceInputRequestTypeDef
...
async with session.create_client("verifiedpermissions") as client:
    ...
    kwargs: CreateIdentitySourceInputRequestTypeDef = {...}
    result: CreateIdentitySourceOutputTypeDef = await client.create_identity_source(**kwargs)
```
