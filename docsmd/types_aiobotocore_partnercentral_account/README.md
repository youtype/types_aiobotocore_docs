...
Install `types-aiobotocore` for `PartnerCentralAccountAPI` service.

```bash
# install with aiobotocore type annotations
python -m pip install 'types-aiobotocore[partnercentral-account]'

# Lite version does not provide session.client/resource overloads
# it is more RAM-friendly, but requires explicit type annotations
python -m pip install 'types-aiobotocore-lite[partnercentral-account]'
...
```bash
python -m pip uninstall -y types-aiobotocore-partnercentral-account
```
...
Type annotations and code completion for  `#!python session.create_client("partnercentral-account")` as [PartnerCentralAccountAPIClient](./client.md)
...
from `#!python session.create_client("partnercentral-account").get_paginator("...")`.
...
def get_list_connection_invitations_paginator() -> ListConnectionInvitationsPaginator:
    return client.get_paginator("list_connection_invitations"))
```
