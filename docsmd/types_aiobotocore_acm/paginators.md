...
Type annotations and code completion for `#!python session.create_client("acm").get_paginator("search_certificates")`.
...
from types_aiobotocore_acm.paginator import SearchCertificatesPaginator

session = get_session()
async with session.create_client("acm") as client:  # (1)
    paginator: SearchCertificatesPaginator = client.get_paginator("search_certificates")  # (2)
    async for item in paginator.paginate(...):
        item: SearchCertificatesResponseTypeDef
        print(item)  # (3)
```

1. client: [ACMClient](./client.md)
2. paginator: [SearchCertificatesPaginator](./paginators.md#searchcertificatespaginator)
3. item: `AioPageIterator[SearchCertificatesResponseTypeDef]`
...
) -> aiobotocore.paginate.AioPageIterator[SearchCertificatesResponseTypeDef]:  # (5)
    ...
```
...
5. See `AioPageIterator[SearchCertificatesResponseTypeDef]`
