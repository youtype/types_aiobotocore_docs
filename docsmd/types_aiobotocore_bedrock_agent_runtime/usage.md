...
async with session.create_client("bedrock-agent-runtime") as client:  # (1)
    result = await client.get_agent_memory()  # (2)
```

1. client: [AgentsforBedrockRuntimeClient](./client.md)
2. result: [:material-code-braces: GetAgentMemoryResponseTypeDef](./type_defs.md#getagentmemoryresponsetypedef) 
...
from types_aiobotocore_bedrock_agent_runtime.type_defs import GetAgentMemoryRequestRequestTypeDef
...
async with session.create_client("bedrock-agent-runtime") as client:
    ...
    result: GetAgentMemoryResponseTypeDef = await client.get_agent_memory(**kwargs)
```
