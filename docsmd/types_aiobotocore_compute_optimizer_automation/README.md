...
Install `types-aiobotocore` for `ComputeOptimizerAutomation` service.

```bash
# install with aiobotocore type annotations
python -m pip install 'types-aiobotocore[compute-optimizer-automation]'

# Lite version does not provide session.client/resource overloads
# it is more RAM-friendly, but requires explicit type annotations
python -m pip install 'types-aiobotocore-lite[compute-optimizer-automation]'
...
```bash
python -m pip uninstall -y types-aiobotocore-compute-optimizer-automation
```
...
Type annotations and code completion for  `#!python session.create_client("compute-optimizer-automation")` as [ComputeOptimizerAutomationClient](./client.md)
...
from `#!python session.create_client("compute-optimizer-automation").get_paginator("...")`.
