...
Type annotations and code completion for `#!python session.create_client("bcm-dashboards").get_paginator("list_scheduled_reports")`.
...
from types_aiobotocore_bcm_dashboards.paginator import ListScheduledReportsPaginator

session = get_session()
async with session.create_client("bcm-dashboards") as client:  # (1)
    paginator: ListScheduledReportsPaginator = client.get_paginator("list_scheduled_reports")  # (2)
    async for item in paginator.paginate(...):
        item: ListScheduledReportsResponseTypeDef
        print(item)  # (3)
```

1. client: [BillingandCostManagementDashboardsClient](./client.md)
2. paginator: [ListScheduledReportsPaginator](./paginators.md#listscheduledreportspaginator)
3. item: `AioPageIterator[ListScheduledReportsResponseTypeDef]`
...
def paginate(
    ...
    *,
    PaginationConfig: PaginatorConfigTypeDef = ...,  # (1)
) -> aiobotocore.paginate.AioPageIterator[ListScheduledReportsResponseTypeDef]:  # (2)
    ...
```

1. See [:material-code-braces: PaginatorConfigTypeDef](./type_defs.md#paginatorconfigtypedef)
2. See `AioPageIterator[ListScheduledReportsResponseTypeDef]`
