...
async with session.create_client("omics") as client:  # (1)
    result = await client.batch_delete_read_set()  # (2)
```
...
from types_aiobotocore_omics.type_defs import BatchDeleteReadSetRequestRequestTypeDef
...
async with session.create_client("omics") as client:
    ...
    result: BatchDeleteReadSetResponseTypeDef = await client.batch_delete_read_set(**kwargs)
```
