...
Type annotations and code completion for `#!python session.create_client("batch").get_paginator("list_quota_shares")`.
...
from types_aiobotocore_batch.paginator import ListQuotaSharesPaginator

session = get_session()
async with session.create_client("batch") as client:  # (1)
    paginator: ListQuotaSharesPaginator = client.get_paginator("list_quota_shares")  # (2)
    async for item in paginator.paginate(...):
        item: ListQuotaSharesResponseTypeDef
        print(item)  # (3)
```

1. client: [BatchClient](./client.md)
2. paginator: [ListQuotaSharesPaginator](./paginators.md#listquotasharespaginator)
3. item: `AioPageIterator[ListQuotaSharesResponseTypeDef]`
...
def paginate(
    ...
    *,
    jobQueue: str,
    PaginationConfig: PaginatorConfigTypeDef = ...,  # (1)
) -> aiobotocore.paginate.AioPageIterator[ListQuotaSharesResponseTypeDef]:  # (2)
    ...
```

1. See [:material-code-braces: PaginatorConfigTypeDef](./type_defs.md#paginatorconfigtypedef)
2. See `AioPageIterator[ListQuotaSharesResponseTypeDef]`
