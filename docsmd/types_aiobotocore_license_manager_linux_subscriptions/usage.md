...
async with session.create_client("license-manager-linux-subscriptions") as client:  # (1)
    result = await client.list_linux_subscription_instances()  # (2)
```
...
from types_aiobotocore_license_manager_linux_subscriptions.type_defs import ListLinuxSubscriptionInstancesRequestRequestTypeDef
...
async with session.create_client("license-manager-linux-subscriptions") as client:
    ...
    result: ListLinuxSubscriptionInstancesResponseTypeDef = await client.list_linux_subscription_instances(**kwargs)
```
