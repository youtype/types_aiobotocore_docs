...
Type annotations and code completion for `#!python session.create_client("verifiedpermissions").get_paginator("list_policy_store_aliases")`.
...
from types_aiobotocore_verifiedpermissions.paginator import ListPolicyStoreAliasesPaginator

session = get_session()
async with session.create_client("verifiedpermissions") as client:  # (1)
    paginator: ListPolicyStoreAliasesPaginator = client.get_paginator("list_policy_store_aliases")  # (2)
    async for item in paginator.paginate(...):
        item: ListPolicyStoreAliasesOutputTypeDef
        print(item)  # (3)
```

1. client: [VerifiedPermissionsClient](./client.md)
2. paginator: [ListPolicyStoreAliasesPaginator](./paginators.md#listpolicystorealiasespaginator)
3. item: `AioPageIterator[ListPolicyStoreAliasesOutputTypeDef]`
...
def paginate(
    ...
    *,
    ...
    PaginationConfig: PaginatorConfigTypeDef = ...,  # (2)
) -> aiobotocore.paginate.AioPageIterator[ListPolicyStoreAliasesOutputTypeDef]:  # (3)
    ...
```

1. See [:material-code-braces: PolicyStoreAliasFilterTypeDef](./type_defs.md#policystorealiasfiltertypedef)
2. See [:material-code-braces: PaginatorConfigTypeDef](./type_defs.md#paginatorconfigtypedef)
3. See `AioPageIterator[ListPolicyStoreAliasesOutputTypeDef]`
