...
Type annotations and code completion for `#!python session.create_client("cloudtrail").get_paginator("list_insights_data")`.
...
from types_aiobotocore_cloudtrail.paginator import ListInsightsDataPaginator

session = get_session()
async with session.create_client("cloudtrail") as client:  # (1)
    paginator: ListInsightsDataPaginator = client.get_paginator("list_insights_data")  # (2)
    async for item in paginator.paginate(...):
        item: ListInsightsDataResponseTypeDef
        print(item)  # (3)
```

1. client: [CloudTrailClient](./client.md)
2. paginator: [ListInsightsDataPaginator](./paginators.md#listinsightsdatapaginator)
3. item: `AioPageIterator[ListInsightsDataResponseTypeDef]`
...
) -> aiobotocore.paginate.AioPageIterator[ListInsightsDataResponseTypeDef]:  # (4)
    ...
```
...
4. See `AioPageIterator[ListInsightsDataResponseTypeDef]`
