...
async with session.create_client("bedrock") as client:  # (1)
    paginator = client.get_paginator("list_custom_model_deployments")  # (2)
...
3. item: [:material-code-braces: ListCustomModelDeploymentsResponseTypeDef](./type_defs.md#listcustommodeldeploymentsresponsetypedef)
...
async with session.create_client("bedrock") as client:
    client: BedrockClient
    paginator: ListCustomModelDeploymentsPaginator = client.get_paginator("list_custom_model_deployments")
    async for item in paginator.paginate(...):
