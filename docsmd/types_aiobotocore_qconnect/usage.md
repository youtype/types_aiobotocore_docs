...
async with session.create_client("qconnect") as client:  # (1)
    result = await client.create_ai_agent()  # (2)
```

1. client: [QConnectClient](./client.md)
2. result: [:material-code-braces: CreateAIAgentResponseTypeDef](./type_defs.md#createaiagentresponsetypedef) 
...
from types_aiobotocore_qconnect.type_defs import CreateAIAgentRequestRequestTypeDef
...
async with session.create_client("qconnect") as client:
    ...
    kwargs: CreateAIAgentRequestRequestTypeDef = {...}
    result: CreateAIAgentResponseTypeDef = await client.create_ai_agent(**kwargs)
```
