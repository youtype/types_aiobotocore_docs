...
Install `types-aiobotocore` for `WickrAdminAPI` service.

```bash
# install with aiobotocore type annotations
python -m pip install 'types-aiobotocore[wickr]'

# Lite version does not provide session.client/resource overloads
# it is more RAM-friendly, but requires explicit type annotations
python -m pip install 'types-aiobotocore-lite[wickr]'
...
```bash
python -m pip uninstall -y types-aiobotocore-wickr
```
...
Type annotations and code completion for  `#!python session.create_client("wickr")` as [WickrAdminAPIClient](./client.md)
...
from `#!python session.create_client("wickr").get_paginator("...")`.
...
def get_list_blocked_guest_users_paginator() -> ListBlockedGuestUsersPaginator:
    return client.get_paginator("list_blocked_guest_users"))
```
