...
Install `types-aiobotocore` for `MarketplaceDiscovery` service.

```bash
# install with aiobotocore type annotations
python -m pip install 'types-aiobotocore[marketplace-discovery]'

# Lite version does not provide session.client/resource overloads
# it is more RAM-friendly, but requires explicit type annotations
python -m pip install 'types-aiobotocore-lite[marketplace-discovery]'
...
```bash
python -m pip uninstall -y types-aiobotocore-marketplace-discovery
```
...
Type annotations and code completion for  `#!python session.create_client("marketplace-discovery")` as [MarketplaceDiscoveryClient](./client.md)
...
from `#!python session.create_client("marketplace-discovery").get_paginator("...")`.
...
def get_get_offer_terms_paginator() -> GetOfferTermsPaginator:
    return client.get_paginator("get_offer_terms"))
```
