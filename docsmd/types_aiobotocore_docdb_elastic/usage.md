...
async with session.create_client("docdb-elastic") as client:  # (1)
    result = await client.copy_cluster_snapshot()  # (2)
```

1. client: [DocDBElasticClient](./client.md)
2. result: [:material-code-braces: CopyClusterSnapshotOutputTypeDef](./type_defs.md#copyclustersnapshotoutputtypedef) 
...
from types_aiobotocore_docdb_elastic.type_defs import CopyClusterSnapshotInputRequestTypeDef
...
async with session.create_client("docdb-elastic") as client:
    ...
    result: CopyClusterSnapshotOutputTypeDef = await client.copy_cluster_snapshot(**kwargs)
```
